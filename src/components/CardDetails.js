      ...
      <div className='modal'>
        ...
        <div className="cardDetails">
          <p className='modaltext'>{currentCards[selectedCard].supertype}</p>
            ...
            {collectionMounted && <p>{cardTotalValue(currentCards[selectedCard])}</p>}
            ...
            {selectedCard && Object.keys(currentCards[selectedCard].tcgplayer.prices).map(v => (
              <React.Fragment key={`${v}Fragment`}>
                ...
                {/* vv takes the variation from the card object, splits up each word, trims off excess space at the ends, then capitalizes the first letter of each word ( REWORK INTO SEPARATE FUNCTION vvv )*/}
                  ...
                  {collectionMounted &&
                    <p>
                      <span>You Own: </span><span className='addOrRemoveBtn' onClick={() => decrementCardVariation(currentCards[selectedCard], v)}>-</span>{showCollectionAmount(currentCards[selectedCard].id, v)}<span className='addOrRemoveBtn' onClick={() => saveLocalCollection(currentCards[selectedCard], v)}>+</span></p>
                  }
                  {/* if collection component isn't mounted, show the 'add to collection' button */}
                  {!collectionMounted && <button className='addToCollection' key={`${v}Btn`} onClick={() => saveLocalCollection(currentCards[selectedCard], v)}>Add to Collection</button>}
                  {/* if collection component is mounted, show the total value of that card in your collection */}
                  {collectionMounted && <p>Total Value: {totalVariationValue(currentCards[selectedCard], v)}</p>}
                  {/* {cardTotalValue(currentCards[selectedCard])} */}
